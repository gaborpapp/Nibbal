...
namespace bullet
{
	...
	VboMeshRef VboMeshManager::createTorus( uint32_t segments )
	{
		// Declare vectors
		vector<uint32_t> indices;
		vector<Vec3f> normals;
		...
		{
			ct = math<float>::cos( 2.0f * (float)M_PI * (float)i / (float)( segments - 1 ));
			st = math<float>::sin( 2.0f * (float)M_PI * (float)i / (float)( segments - 1 ));

			for( j = 0; j < segments; j++ )
			{
				cp =  math<float>::cos( 2.0f * (float)M_PI * (float)j / (float)( segments - 1 ));
				sp =  math<float>::sin( 2.0f * (float)M_PI * (float)j / (float)( segments - 1 ));

				normals.push_back( Vec3f( cp * ct, sp * ct, st ));

				texCoords.push_back( Vec2f( 1.0f * (float)i / (float)( segments - 1 ), 5.0f * (float)j / (float)( segments - 1 )));

				positions.push_back( Vec3f( cp * ( radiusOut  + radiusIn * ct ), sp * ( radiusOut  + radiusIn * ct ), radiusIn * st ));
			}
		}

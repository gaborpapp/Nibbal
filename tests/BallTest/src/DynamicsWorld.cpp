...
namespace bullet {
	...
	CollisionObjectRef createRigidTorus( const DynamicsWorldRef &world, float radiusIn, float radiusOut, int32_t segments, float mass, 
		const ci::Vec3f &position, const ci::Quatf &rotation )
	{
		return world->pushBack( new RigidTorus( radiusIn, radiusOut, segments, mass, position, rotation ) );
	}
